...
# quantiles_v <- quantile(bd_mice$SAL_SEM, probs = seq(0, 1, 0.2))
# id_quant_val <- which(bd_mice$SAL_SEM>=quantiles_v[2] & bd_mice$SAL_SEM<=quantiles_v[5])
# id_quant_val <- which(bd_mice$SAL_SEM<=quantiles_v[5])
...
fn_modelado <- function(datos){
  ...
  for(j in 1:32){
    ...
    # ==============================================================================
    table_PAREN <- table(datos_ENT_j$PAREN)
    id_elim <- which(table_PAREN<15)
    if(length(id_elim)>0){
      oficio_elim <- names(table_PAREN)[id_elim]
      id_elim <- list()
      k=1
      for(k in 1:length(oficio_elim)){
        id_elim[[k]] <- which(oficio_elim[k]==datos_ENT_j$PAREN)
      }
      datos_ENT_j <- datos_ENT_j[-unlist(id_elim), ]
    }
    ...
    ###
    ### SE CONSTRUYE LA FORMULA: INTERACCIONES ORDEN 2
    ###
    par_var_x <- colnames(datos)[-c(1, grep("ENT", colnames(datos)))]
    ...
    print(c("ent: ",j, "0_vars_formula: ",length(x_formula_combinacion_i_ls)+1))
    
    ###
    ### 0) METODO: Mínimos cuadrados (OLS)
    ###
    ...
    model_summary <- modelo_ols_results$summary_i$coefficients
    model_summary <- as.data.frame(model_summary)
    id_vars_sig <- which(model_summary$`Pr(>|t|)`<0.05)
    vars_sig <- strsplit(rownames(model_summary[id_vars_sig,])[-1], ":")
    vars_sig_ls <- c()
    p=3
    for(p in 1:length(vars_sig)){
      print(p)
      vars_sig_v <- vars_sig[[p]][1]
      id_col <- agrep(vars_sig_v, colnames(datos_train))
      vars_sig_ls[p] <- id_col
    }
    vars_significativas_ols <- unique(colnames(datos_train)[vars_sig_ls])
    vars_sig_unique <- unique(vars_sig_ls)
    # length(vars_significativas_ols)
    # length(colnames(datos_train))
    
    
    ###
    ### SE GENERA UN GRID NUEVO CON LAS VARIABLES SELECCIONADAS
    ###
    
    par_var_x <- colnames(datos_train)[c(vars_sig_unique)]
    ### GRID DE COMBINACIONES DE VARIABLES REGRESORAS
    ncols_vars_x <- length(par_var_x)
    l <- rep(list(0:1), ncols_vars_x)
    grid_vars_x_2 <- expand.grid(l)
    id_elim <- which(apply(grid_vars_x_2, 1, FUN=sum)==0)
    grid_vars_x_2 <- grid_vars_x_2[-id_elim,]
    
    id_pares <- which(apply(grid_vars_x_2, 1, FUN=sum)<=2)
    grid_vars_x_2 <- grid_vars_x_2[id_pares,]
    
    ###
    ### LOOP INTERACCIONES ORDEN 2
    ###
    x_formula <- paste(vars_significativas_ols, collapse="+")
    x_formula_combinacion_i_ls <- list()
    i=1
    for(i in 1:nrow(grid_vars_x_2)){
      
      ###
      ### SE CONSTRUYE LA FORMULA
      ###
      
      # print(c(i, nrow(grid_vars_x_2)))
      id_cols_i <- which(grid_vars_x_2[i,]==1)
      if(length(id_cols_i)==1){
        x_formula_combinacion_i <- paste(par_var_x[id_cols_i], par_var_x[id_cols_i],sep="*")
      }else{
        x_formula_combinacion_i <- paste(par_var_x[id_cols_i], collapse="*")
    ...
    print(c("ent: ",j, "1_vars_formula: ",length(x_formula_combinacion_i_ls)+1))
    ...
    modelo_ols <- function(formula_i, 
      ...
      # dim(datos_test_)
      # dim(datos_train_)
      predicciones_test <- predict(modelo, newdata = datos_test_[1,])
      length(predicciones_test)
    ...
    # modelo_ridge_results$summary_i
    ...
    modelo_lasso_results <- modelo_lasso(formula_i, 
                                         datos_train, 
                                         datos_test)
    ...
    # ###
    {
      ...
      datos_train_h20 <- datos_train[sample(1:nrow(datos_train), (par_sample_h20*nrow(datos_train)), F),]
      datos_test_h20 <- datos_test[sample(1:nrow(datos_test), (par_sample_h20*nrow(datos_test)), F),]
      # dim(datos_train_h20)
      # dim(datos_test_h20)
      fn_h2o <- function(datos__,
        ...
        ### OJO! NO ES NECESARIO USAR UN DATASET DE PRUEBA, EL MISMO ALGORITMO LO REALIZA Y
        ### ARROJA ESTADISTICOS DE CALIDAD DEL MODELO

        # ?h2o.performance
        # rf_perf1 <- h2o.performance(model = m, newdata = datos_)
        # rf_perf1
        # predictions <- h2o.predict(m, datos_)
        # test_mse_ols <- mean((predictions - datos_[,1])^2)
        # paste("Error (mse) de test:", test_mse_ols)
      ...
      h2o_model_name <- fn_h2o_res$best_model@algorithm
    ...
    results_ls$modelos[[j]] <- list(modelo_ols_results$modelo, modelo_step_results$modelo, modelo_lasso_results$modelo, modelo_ridge_results$modelo)
...
### FUNCION
fn_prediction <- function(x_new, x_new_lasso_ridge, codigo_entidad, datos_test_prediction){
  
  # datos_test_prediction <- datos
  
  ### 3. Seleccion del modelo con menor MSE
  
  id_modelo_optimo <- which(fn_modelado_res$mse[[codigo_entidad]]$mse == min(fn_modelado_res$mse[[codigo_entidad]]$mse))
  modelo_optimo_name <- fn_modelado_res$mse[[codigo_entidad]]$modelo[id_modelo_optimo]
  modelo_optimo <- fn_modelado_res$modelos[[codigo_entidad]][[id_modelo_optimo]] ## string
  
  ### 4. Prediccion con el mejor modelo seleccionado
  
  if(modelo_optimo_name=="OLS"){
    prediction <- predict(modelo_ols_results$modelo, newdata = x_new)
  }
  if(modelo_optimo_name=="Stepwise"){
    prediction <- predict(modelo_step_results$modelo, newdata = x_new)
  }
  if(modelo_optimo_name=="Lasso"){
    prediction <- predict(modelo_lasso_results$modelo, newx = x_new_lasso_ridge)
  }
  if(modelo_optimo_name=="Ridge"){
    prediction <- predict(modelo_ridge_results$modelo, newx = x_new_lasso_ridge)
  }
  ...
  mse_modelo_optimo <- fn_modelado_res$mse[[codigo_entidad]]$mse[id_modelo_optimo]
  id_r2_optimo <- grep(modelo_optimo_name, fn_modelado_res$r2[[codigo_entidad]]$modelo)
  r2_modelo_optimo <- fn_modelado_res$r2[[codigo_entidad]]$r2[id_r2_optimo]
  
  ### Se genera un solo Data frame con los resultados principales
  resultado <- data.frame(codigo_entidad=codigo_entidad, modelo_optimo_name, prediccion_final, mse_modelo_optimo, r2_modelo_optimo)
  colnames(resultado)[1] <- "prediccion"
...
### PARAMETROS
# 1. Se define el codigo de la entidad
codigo_entidad <- sample(1:32, 1, F)
# 2. Se CAPTURAN y estructura los nuevos datos para predecir la variable "Y"
# se ejemplifica con un renglon elegido aleatoriamente de la base de datos de prueba
id_row_prueba <- sample(1:nrow(datos_test_prediction), 1, F)
## a) formato "normal" para modelos OLS y STEP
x_new <- datos_test_prediction[id_row_prueba,,drop=F] ## debe llenarse con los datos nuevos (parametros del usuario). Para ejemplificar se considera una de las observaciones de la base de datos
## b) formato de Data frame para modelos "Ridge" y "Lasso"
x_new_lasso_ridge <- model.matrix(formula_i, data = datos_test_prediction)[id_row_prueba,-1, drop=F]

### PREDICCION
prediccion <- fn_prediction(x_new, x_new_lasso_ridge, codigo_entidad, datos_test_prediction)
