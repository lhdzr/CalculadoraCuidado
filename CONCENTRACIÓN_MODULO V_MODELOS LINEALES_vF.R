...
# id_col_elim <- which(lapply(bd, FUN=function(x){length(names(table(x)))})==1)
# bd <- bd[,-id_col_elim]
bd <- bd[which(bd$SAL_SEM>0),]

# ### se consideran los valores mayores a 0
# length(which(bd$Ingreso_mensual>0))
# # dim(bd)
# bd <- bd[which(bd$Ingreso_mensual>0),]
...
# bd_mice <- data.frame(Ingreso_mensual=bd_mice[,grep("Ingreso_mensual", colnames(bd_mice))], bd_mice[,-grep("Ingreso_mensual", colnames(bd_mice))])
# head(bd_mice)
...
fn_prediction <- function(x_new, 
  ...
  id_modelo_optimo <- which(fn_modelado_res$mse[[par_ENT]]$mse == min(fn_modelado_res$mse[[par_ENT]]$mse))
  ...
  a <- boxplot(datos$Ingreso_mensual, main="Predicción con relación a la distribución del ingreso nacional")
  print(a)
  b <- points(prediccion$exp.predict., col=2, lwd=5, pch=19)
...
save.image("equipo_1_modelado.R")
...
pars_usuario_t$Ciudad<- as.factor(pars_usuario_t$Ciudad)
pars_usuario_t$Entidad <- as.factor(pars_usuario_t$Entidad)
pars_usuario_t$Rural <- as.factor(pars_usuario_t$Rural)
pars_usuario_t$Tipo.de.entrevista <- as.factor(pars_usuario_t$Tipo.de.entrevista)
pars_usuario_t$Sexo <- as.factor(pars_usuario_t$Sexo)
pars_usuario_t$Entidad_nacimiento <- as.factor(pars_usuario_t$Entidad_nacimiento)
pars_usuario_t$Sabe_leer_y_escribir <- as.factor(pars_usuario_t$Sabe_leer_y_escribir)
pars_usuario_t$Ultimo_grado_escolaridad <- as.factor(pars_usuario_t$Ultimo_grado_escolaridad)
pars_usuario_t$Asiste_escuela <- as.factor(pars_usuario_t$Asiste_escuela)
pars_usuario_t$Estado_conyugal <- as.factor(pars_usuario_t$Estado_conyugal)
pars_usuario_t$Posicion_ocupacion <- as.factor(pars_usuario_t$Posicion_ocupacion)
pars_usuario_t$Acceso_salud <- as.factor(pars_usuario_t$Acceso_salud)
pars_usuario_t$Sector_economico <- as.factor(pars_usuario_t$Sector_economico)
pars_usuario_t$Formalidad <- as.factor(pars_usuario_t$Formalidad)
pars_usuario_t$Oficio <- as.factor(pars_usuario_t$Oficio)

# (OPCIONAL) Con una muestra de la Base de datos
...
pars_usuario_t_lasso_ridge <- model.matrix(formula_i, data = datos[id_sample,])[,-1, drop=F]
