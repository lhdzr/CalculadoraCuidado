setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/1T/TEC 2022 - Concentración/RETOS/Equipo 1")
# ==============================================================================
library(faraway)
...
# # bd <- read.csv("~/Library/Mobile DocumENTs/com~apple~CloudDocs/Desktop/1T/TEC 2022 - ConcENTración/Retos/Equipo 1/bd2.csv")
# bd <- read.csv("BaseMLOficio.csv")
# 
# bd <- bd[sample(1:nrow(bd), 0.7*nrow(bd), F),]
# 
# ## REORDENAMIENTO DE LAS COLUMNAS
# id_y <- grep("ing_x_hrs", colnames(bd))
...
# bd$rural <- as.factor(bd$rural)
# bd$mujer <- as.factor(bd$mujer)
# bd$emp_ppal <- as.factor(bd$emp_ppal)
# bd$Oficio <- as.factor(bd$Oficio)
# 
...
save.image("equipo_1_environment.R")
# getwd()
